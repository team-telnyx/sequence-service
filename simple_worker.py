...
SCOUT_DB_URL = os.getenv(
    "SCOUT_DATABASE_URL", "postgresql://kevinward@localhost:5432/scout"
)
...
DEFAULT_SEQUENCE_ID = os.getenv(
    "DEFAULT_SEQUENCE_ID", "03510577-09d1-4879-87a0-7b8b76c2a60b"
)
...
def get_timezone_for_email(email: str) -> str | None:
    ...
    for tld, tz in sorted(
        APAC_TLD_TIMEZONES.items(), key=lambda x: len(x[0]), reverse=True
    ):
        if domain.endswith(tld):
            return tz
...
async def get_unenrolled_prospects(scout_pool) -> list:
    ...
    for row in rows:
        ...
        if email not in enrolled_emails:
            ...
            try:
                cadence = (
                    json.loads(prospect["cadence_config"])
                    if prospect["cadence_config"]
                    else {}
                )
...
async def enroll_in_sequence_service(prospect: dict) -> bool:
    ...
    contact_name = " ".join(
        filter(None, [prospect.get("first_name", ""), prospect.get("last_name", "")])
...
async def run_cycle(scout_pool) -> dict:
    ...
    try:
        ...
        logger.info(
            f"Found {len(unenrolled)} prospects needing sequence_service enrollment"
        )
