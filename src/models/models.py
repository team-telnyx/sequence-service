...
class Sequence(Base):
    ...
    status: Mapped[SequenceStatus] = mapped_column(
        Enum(SequenceStatus), default=SequenceStatus.DRAFT
    )
...
class SequenceEnrollment(Base):
    ...
    mailbox_id: Mapped[str] = mapped_column(
        ForeignKey("mailboxes.id")
    )  # Sticky sender - assigned at enrollment
    contact_email: Mapped[str] = mapped_column(String(255))
    contact_name: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    timezone: Mapped[str] = mapped_column(
        String(63), default="America/New_York", server_default="America/New_York"
    )
...
class SuppressionReason(str, enum.Enum):
    ...
    COMPLAINT = "COMPLAINT"
    MANUAL = "MANUAL"
    # REVOPS-1552: origin-split reasons for Email API webhook events. These
    # identify suppressions originating from Telnyx Email API delivery webhooks
    # (bounce/complaint/one-click-unsubscribe), distinct from the reply-
    # classification-driven BOUNCE/COMPLAINT/UNSUBSCRIBE values (poll_replies)
    # and the operator/SFDC MANUAL value. One-way sync: API events write IN;
    # manual/SFDC rows are never modified or written back out. The send path
    # (sequence_step.py) consults the suppressions table BEFORE the transport
    # branch, so both email_api and gmail paths honor these.
    API_BOUNCE = "API_BOUNCE"
    API_COMPLAINT = "API_COMPLAINT"
...
class Suppression(Base):
    ...
    source_enrollment_id: Mapped[Optional[str]] = mapped_column(
        ForeignKey("sequence_enrollments.id"), nullable=True
    )
...
class ProcessedEmailEvent(Base):
    """Deduplication marker for Telnyx Email API webhook events (REVOPS-1552).

    Telnyx redelivers events on retry; the event id is unique per logical
    event. The ``id`` (inherited str PK) is set to the Telnyx event id, so a
    redelivery that hits an existing PK is a no-op — the service checks for
    the marker first and skips if present. The marker is written ONLY after
    all event processing succeeds (same transaction), so a partial failure
    leaves no marker and the redelivery retries cleanly. Pruned to
    DEDUPE_RETENTION_DAYS on each event in the service layer so growth stays
    bounded without a cron dependency.
    """
